...
HASH=$(wget https://launchermeta.mojang.com/v1/packages/1e185d112e2ee017330fb83700b5eaa93fb5cdfd/1.16.json -O - | jq -r ".objects.\"minecraft/lang/$1.json\".hash")

mkdir -p lang

wget http://resources.download.minecraft.net/$(echo $HASH | head -c 2)/$HASH -O - | jq -r 'to_entries|map("\(.key)=\(.value)")[]' > lang/$(echo $1 | cut -d '_' -f 1)_$LANG_SUF.lang
